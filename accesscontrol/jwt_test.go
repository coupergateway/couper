...
func Test_JWT_Validate(t *testing.T) {
	...
	for _, signingMethod := range []jwt.SigningMethod{
		...
		for _, tt := range tests {
			t.Run(fmt.Sprintf("%v_%s", signingMethod, tt.name), func(st *testing.T) {
				...
				if err = j.Validate(tt.req); (err != nil) != tt.wantErr {
					st.Errorf("Validate() error = %v, wantErr %v", err, tt.wantErr)
				}
				...
				if !tt.wantErr && tt.fields.claims != nil {
					...
					if claims, ok := acMap["test_ac"]; !ok {
						st.Errorf("Expected a configured access control name within request context")
					} else {
						claimsMap := claims.(map[string]interface{})
						for k, v := range tt.fields.claims {
							if claimsMap[k] != v {
								st.Errorf("Claim does not match: %q want: %v, got: %v", k, v, claimsMap[k])
							}
						}
					}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(st *testing.T) {
			...
			if !tt.wantErr {
				...
				if !ok {
					st.Errorf("Expected scopes within request context")
				} else {
					for i, v := range expScopes {
						if scopesList[i] != v {
							st.Errorf("Scopes do not match, want: %v, got: %v", v, scopesList[i])
						}
					}
				}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(st *testing.T) {
			...
			if expectedError != error {
				st.Errorf("%q: Unexpected configuration error:\n\tWant: %q\n\tGot:  %q", tt.name, expectedError, error)
			}
		})
	}
}
