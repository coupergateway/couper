...
func Test_realmain(t *testing.T) {
	...
	tests := []struct {
		...
		// -p 0: the only case which listens, and the test packages run in parallel.
		{"path from env /w missing key", []string{"couper", "run", "-f", "public/couper.hcl", "-f", base + "/no_key_from_env.hcl", "-p", "0"}, nil, "", 0},
		...
		{"throttle block in anonymous backend", []string{"couper", "run", "-f", base + "/17_couper.hcl"}, nil, `level=error msg="configuration error: anonymous_3_11: anonymous backend 'anonymous_3_11' cannot define 'throttle' block(s)" build=dev`, 1},
