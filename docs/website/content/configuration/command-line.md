...
| `-beta-service-name`             | `couper`            | `COUPER_BETA_SERVICE_NAME`             | The service name which applies to the `service_name` metric and trace labels.                                                                                                                                                               |
| `-beta-traces`                   | `false`             | `COUPER_BETA_TRACES`                   | Enables the OpenTelemetry [traces](/observation/tracing) exporter.                                                                                                                                                                          |
| `-beta-traces-endpoint`          | `localhost:4317`    | `COUPER_BETA_TRACES_ENDPOINT`          | OpenTelemetry collector endpoint for exporting traces via gRPC.                                                                                                                                                                             |
| `-beta-traces-trust-parent`      | `false`             | `COUPER_BETA_TRACES_TRUST_PARENT`      | If enabled, the `traceparent` request header from an incoming request is used as the parent trace context.                                                                                                                                  |
| `-beta-traces-parent-only`       | `false`             | `COUPER_BETA_TRACES_PARENT_ONLY`       | If enabled, Couper only creates trace spans for requests that carry a `traceparent` header.                                                                                                                                                 |
