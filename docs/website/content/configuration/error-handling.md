...
For this purpose every access control definition of `basic_auth`, `beta_authzen`, `jwt`, `oidc` or `saml2` can define one or multiple [`error_handler` blocks](/configuration/block/error_handler) with one or more defined error type labels listed below.
...
The following table documents error types that can be handled in the respective access control blocks (`basic_auth`, `beta_authzen`, `jwt`, `saml`, `beta_oauth2`, `oidc`):
...
| `authzen` (`access_control`)             | Couper got no decision, e.g. a callout failure, an error status of the authorization service, or a response without a `decision`. | Send error template with status `403`.                                      |
| `authzen_invalid_credentials` (`authzen`) | The authorization service denied the request and sent a `www_authenticate` challenge in its response context.        | Send error template with status `401`.                                      |
| `authzen_insufficient_permissions` (`authzen`) | The authorization service denied the request without a challenge.                                                | Send error template with status `403`.                                      |
...
| `beta_backend_token_request` (`backend`)           | A token request for the backend has failed.                                                             | Send error template with status `502`.                                                                        |
| `access_control`                                   | Access control related errors.                                                                          | Send error template with status `403`.                                                                        |
| `backend_throttle_exceeded`                        | Backend throttle related errors.                                                                        | Send error template with status `429`.                                                                        |
| `insufficient_permissions` (`access_control`)      | The permission required for the requested operation is not in the permissions granted to the requester. | Send error template with status `403`.                                                                        |
