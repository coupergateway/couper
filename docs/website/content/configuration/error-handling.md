...
| `external_authz` (`access_control`)             | Couper got no decision, e.g. a callout failure, an error status of the authorization service, or a response without a `decision`. | Send error template with status `403`.                                      |
| `external_authz_invalid_credentials` (`external_authz`) | The authorization service denied the request and sent a `www_authenticate` challenge in its response context.        | Send error template with status `401`.                                      |
| `external_authz_insufficient_permissions` (`external_authz`) | The authorization service denied the request without a challenge.                                                | Send error template with status `403`.                                      |
