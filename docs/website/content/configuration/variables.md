...
For a [`beta_external_authz` block](/configuration/block/beta_external_authz) the variable contains the `context` object of the authorization service's access evaluation response, e.g. validated claims or resolved permissions, plus the `decision` and the response `headers`.
