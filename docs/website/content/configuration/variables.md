...
For a [`beta_authzen` block](/configuration/block/beta_authzen) the variable contains the `context` object of the authorization service's access evaluation response, e.g. validated claims or resolved permissions, plus the `decision` and the response `headers`.
