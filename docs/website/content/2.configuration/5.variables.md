...
| `context.granted_permissions`      | list (string) | Permissions granted to the requester as yielded by access controls (see e.g. `permissions_claim`, `beta_roles_claim` in the [`jwt` block](/configuration/block/jwt)).                | `["perm1", "perm2"]`                          |
