...
For a [`basic_auth` block](block/basic_auth) and successfully authenticated request the variable contains the `user` name.
...
For an [`beta_oauth2` block](block/oauth2), the variable contains the response from the token endpoint, e.g.
...
and for an [`oidc` block](block/oidc) additionally:
...
Only available in the [`beta_token_request` block](block/token_request) context, `beta_token_response` allows access to the current token response (see [`backend_responses`](#backend_responses) for available properties).
...
| `beta_tokens.<token_request_name>` | string | The token obtained by the [token request](block/token_request) with name `<token_request_name>`     |                                                      |
| `beta_token`                       | string | The token obtained by the [token request](block/token_request) with name `"default"`, if configured |                                                      |

## `beta_backend`

Only available in the `backend` block context, `beta_backend` allows access to the current backend information (see [`backends`](#backends) for available properties).
