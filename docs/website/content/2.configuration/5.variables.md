...
| `environment`                    | string | The [environment](/configuration/command-line#global-options) Couper currently runs in    | `"prod"`  |
...
| `context.beta_granted_permissions` | list (string) | Permissions granted to the requester as yielded by access controls (see e.g. `beta_permissions_claim`, `beta_roles_claim` in the [`jwt` block](/configuration/block/jwt)).                | `["perm1", "perm2"]`                          |
