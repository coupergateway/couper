...
Only available in the [`beta_token_request` block](block/token_request) context, `token_response` allows access to the current token response (see [`backend_responses`](#backend_responses) for available properties).
...
| `tokens.<token_request_name>` | string | The token obtained by the [token request](block/token_request) with name `<token_request_name>`     |                                                      |
| `token`                       | string | The token obtained by the [token request](block/token_request) with name `"default"`, if configured |                                                      |
