...
> Note: `e`, `log-format`, `log-level`, `log-pretty`, `pprof` and `pprof-port` also map to [settings](/configuration/block/settings).
...
| `-xfh`                  | `false`      | `COUPER_XFH`                  | Global configurations which uses the `Forwarded-Host` header instead of the request host. |
