...
|           | **Note:** `run` options can also be configured with [settings](/configuration/block/settings) or related environment variables. |
...
| `-pprof`             | –            | `COUPER_PPROF`             | Enables profiling. |
| `-pprof-port`        | `6060`       | `COUPER_PPROF_PORT`        | Port for profiling interface |

> Note: `log-format`, `log-level` and `log-pretty` also map to [settings](/configuration/block/settings).
