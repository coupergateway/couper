...
For this purpose every access control definition of `basic_auth`, `jwt`, `oidc` or `saml2` can define one or multiple [`error_handler` blocks](block/error_handler) with one or more defined error type labels listed below.
...
A [sequence](block/endpoint#endpoint-sequence), a simple `request` or `proxy` error can be handled in combination with the `expected_status` attribute for `request`
and `proxy` block definitions and an [`error_handler` block](block/error_handler) with the [related](#endpoint-error-types) type label.
