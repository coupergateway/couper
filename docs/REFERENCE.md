...
| `expected_status`       | list<number> | -       | If defined, the [Backend Block](#backend-block) response status code will be verified against this list of codes. If the status-code is unexpected an [`unexpected_status` error](./ERRORS.md#error-types) can be handled with an [`error_handler`](./ERRORS.md#endpoint-related-error_handler). | -                                                                                      | -                   |
