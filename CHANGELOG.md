...
* **Added**
  ...
  * default the `beta_external_authz` callout to the AuthZEN access evaluation endpoint `/access/v1/evaluation` when the configured `url` has no path, and send Couper's request id as `X-Request-ID` for log correlation ([#873](https://github.com/coupergateway/couper/issues/873))
