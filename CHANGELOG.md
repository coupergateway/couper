...
* **Security**
  ...
  * [`basic_auth`](https://docs.couper.io/configuration/block/basic_auth): bound the argon2 `htpasswd_file` derivation cost. The parameters in an entry apply to every request, so Couper now limits the derivations that run at the same time to the number of available cores, collapses concurrent verifications of the same credential into one derivation, and warns at startup about an entry above the recommended maxima (`m` ≤ 94208 KiB, `t` ≤ 10, `p` ≤ 2 — 2x the highest OWASP values). The entry still loads, so an upgrade cannot stop a running deployment. Per-attempt cost still follows the parameters in the hash, so chain a [`beta_rate_limiter`](https://docs.couper.io/configuration/block/rate_limiter) before the access control to reject a caller before any derivation runs ([#866](https://github.com/coupergateway/couper/issues/866))
...
* **Dependencies**
  ...
  * `golang.org/x/sync` becomes a direct requirement, for the argon2 flight collapsing ([#1009](https://github.com/coupergateway/couper/pull/1009))
