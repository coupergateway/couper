...
* **Added**
  ...
  * `configuration_url`, `configuration_ttl` and `configuration_max_stale` attributes for `beta_external_authz`: read the callout endpoint from the AuthZEN configuration document (`/.well-known/authzen-configuration`) of the authorization service; Couper verifies the advertised `policy_decision_point` and rejects an endpoint on a foreign origin ([#873](https://github.com/coupergateway/couper/issues/873))
