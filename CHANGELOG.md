...
* **Added**
  * `beta_authzen` access control: delegate the authorization decision to a policy decision point via an [OpenID AuthZEN Authorization API 1.0](https://openid.net/specs/authorization-api-1_0.html) access evaluation callout, with distinct `authzen_invalid_credentials` (401) and `authzen_insufficient_permissions` (403) error types and opt-in TLS connection state (`include_tls`) ([#873](https://github.com/coupergateway/couper/issues/873))
  ...
  * `http2_prior_knowledge` backend attribute: cleartext HTTP/2 (h2c) for trusted `http` origins, e.g. multiplexed `beta_authzen` callouts without TLS ([#873](https://github.com/coupergateway/couper/issues/873))
  * forward the authorization service's challenge on `beta_authzen` denials via a default `error_handler`: a `www_authenticate` property in the access evaluation response context answers `401` instead of `403` and is available as `request.context.<label>.www_authenticate` ([#873](https://github.com/coupergateway/couper/issues/873))
  * expose a `beta_authzen` callout's access evaluation response context, its `decision` and its response headers (`request.context.<label>` / `request.context.<label>.headers`), on an allowed and on a denied request; inject a resolved identity upstream with `set_request_headers` ([#873](https://github.com/coupergateway/couper/issues/873))
  * `configuration_url`, `configuration_ttl` and `configuration_max_stale` attributes for `beta_authzen`: read the callout endpoint from the AuthZEN configuration document (`/.well-known/authzen-configuration`) of the authorization service; Couper verifies the advertised `policy_decision_point` — a multi-tenant decision point may root it under a path of its own, e.g. OpenFGA's `/stores/<store_id>` — and rejects an endpoint on a foreign origin ([#873](https://github.com/coupergateway/couper/issues/873))
  * `evaluate_permissions` attribute for `beta_authzen`: resolve candidate permissions with one batch callout to the AuthZEN access evaluations endpoint `/access/v1/evaluations`, for policy engines that answer boolean questions instead of handing out permission sets ([#873](https://github.com/coupergateway/couper/issues/873))
  * default the `beta_authzen` callout to the AuthZEN access evaluation endpoint `/access/v1/evaluation` when the configured `url` has an empty or root path, and send Couper's request id as `X-Request-ID` for log correlation ([#873](https://github.com/coupergateway/couper/issues/873))
  * `subject`, `action`, `resource` and `context` attributes for `beta_authzen`: shape the access evaluation request per request, e.g. name the subject from a preceding `jwt` access control (`request.context.<label>.sub`) ([#873](https://github.com/coupergateway/couper/issues/873))
  * forward the client certificate identity in a `beta_authzen` callout's `context.tls` (`include_tls`) — subject/issuer DN, `serial_number`, `fingerprint_sha256`, and the subject alternative names (`dns_names`, `uris`, `email_addresses`, `ip_addresses`) — for client-facing mTLS authorization decisions ([#873](https://github.com/coupergateway/couper/issues/873))
