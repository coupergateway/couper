...
* **Security**
  ...
  * [`basic_auth`](https://docs.couper.io/configuration/block/basic_auth): bound the argon2 cost of an `htpasswd_file` entry. The argon2 parameters apply to each request. Couper now runs no more derivations at the same time than it has cores. Concurrent attempts with the same credential share one derivation. Couper warns at startup if an entry is above the recommended maxima (`m` ≤ 94208 KiB, `t` ≤ 10, `p` ≤ 2). The entry still loads. Each attempt still costs what its parameters specify, so put a [`beta_rate_limiter`](https://docs.couper.io/configuration/block/rate_limiter) before this access control ([#866](https://github.com/coupergateway/couper/issues/866))
