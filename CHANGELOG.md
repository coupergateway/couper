...
* **Added**
  ...
  * forward the authorization service's challenge on `beta_external_authz` denials via a default `error_handler`: a `www_authenticate` property in the access evaluation response context answers `401` instead of `403` and is available as `request.context.<label>.www_authenticate` ([#873](https://github.com/coupergateway/couper/issues/873))
  * expose a `beta_external_authz` callout's access evaluation response context, its `decision` and its response headers (`request.context.<label>` / `request.context.<label>.headers`), on an allowed and on a denied request; inject a resolved identity upstream with `set_request_headers` ([#873](https://github.com/coupergateway/couper/issues/873))
  * `configuration_url`, `configuration_ttl` and `configuration_max_stale` attributes for `beta_external_authz`: read the callout endpoint from the AuthZEN configuration document (`/.well-known/authzen-configuration`) of the authorization service; Couper verifies the advertised `policy_decision_point` — a multi-tenant decision point may root it under a path of its own, e.g. OpenFGA's `/stores/<store_id>` — and rejects an endpoint on a foreign origin ([#873](https://github.com/coupergateway/couper/issues/873))
  * `evaluate_permissions` attribute for `beta_external_authz`: resolve candidate permissions with one batch callout to the AuthZEN access evaluations endpoint `/access/v1/evaluations`, for policy engines that answer boolean questions instead of handing out permission sets; mutually exclusive with `permissions_property` ([#873](https://github.com/coupergateway/couper/issues/873))
  * default the `beta_external_authz` callout to the AuthZEN access evaluation endpoint `/access/v1/evaluation` when the configured `url` has an empty or root path, and send Couper's request id as `X-Request-ID` for log correlation ([#873](https://github.com/coupergateway/couper/issues/873))
  * `subject`, `action`, `resource` and `context` attributes for `beta_external_authz`: shape the access evaluation request per request, e.g. name the subject from a preceding `jwt` access control (`request.context.<label>.sub`) ([#873](https://github.com/coupergateway/couper/issues/873))
  * `permissions_property` attribute for `beta_external_authz`: grant permissions for `required_permission` checks from a named property of the authorization service's response context; a configured property missing from an allowed response denies the request ([#873](https://github.com/coupergateway/couper/issues/873))
