...
* **Added**
  ...
  * forward the authorization service's `WWW-Authenticate` challenge on `beta_authz_external` denials via a default `error_handler`; the value is available as `request.context.<label>.www_authenticate` ([#873](https://github.com/coupergateway/couper/issues/873))
  ...
  * a configured `permissions_property` missing from a `beta_authz_external` `200` response now denies the request ([#873](https://github.com/coupergateway/couper/issues/873))
