...
func NewBackend(ctx *hcl.EvalContext, body hcl.Body, log *logrus.Entry, proxy bool,
	store *cache.MemoryStore) (http.RoundTripper, hcl.Body, error) {
	...
	backend, modifiedBody, err := newBackend(ctx, ctxBody, log, proxy, store)
	if err != nil {
		return nil, nil, err
	}
