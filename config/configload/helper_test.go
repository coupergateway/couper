...
func TestRenameBetaBlocks_EmptyFrom(t *testing.T) {
	...
	if innerErr == nil || !strings.Contains(innerErr.Error(), "from cannot be empty") {
...
func TestRenameBetaBlocks_MissingBetaPrefix(t *testing.T) {
	...
	innerErr := errors.Unwrap(err)
	if innerErr == nil || !strings.Contains(innerErr.Error(), "must start with 'beta_'") {
...
func TestRenameBetaBlocks_IncorrectTo(t *testing.T) {
	...
	innerErr := errors.Unwrap(err)
	if innerErr == nil || !strings.Contains(innerErr.Error(), "to must be") {
