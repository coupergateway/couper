...
| COUPER_SECURE_COOKIES | `""` | If set to `"strip"`, the `Secure` flag is removed from all `Set-Cookie` HTTP header fields. If set to `"enforce"` the `Secure` flag is added to all `Set-Cookie` HTTP header fields (if not already included). |
