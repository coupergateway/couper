...
| COUPER_SECURE_COOKIES | `""` | If set to `"strip"`, the `Secure` flag of all `Set-Cookie` HTTP header fileds are removed. If set to `"enforce"` the `Secure` flag is added to all `Set-Cookie` HTTP header fileds (if not already included). |
