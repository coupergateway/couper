...
func TestJWTAccessControl(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		{"local RSA JWKS not found", "/jwks/rsa/not_found", http.Header{"Authorization": []string{"Bearer " + rsaToken}}, "", http.StatusForbidden, "", "access control error: JWKS_not_found: error loading synced JSON: status code 404"},
...
func TestEndpoint_ResponseNilEvaluation(t *testing.T) {
	...
	for _, tc := range []testCase{
		...
		{"/conditional/string?s=", true, "empty"},
